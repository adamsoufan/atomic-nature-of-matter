...
class BlobFinder:
    ...
    def __init__(self, pic, tau):
        ...
        marked = stdarray.create2D(pic.height(), pic.width(), False)
        ...
        for i in range(pic.width()):
            for j in range(pic.height()):
                ...
                # If blob has one or more pixels, add it to self._blobs.
                if blob.mass() > 0:
                    self._blobs += [blob]

    # Returns a list of all beads (blobs with mass >= pixels).
    def getBeads(self, pixels):
        # Set beads to empty list.
        beads = []

        for blob in self._blobs:
            # For each blob in self._blobs...

            # If blob has pixels or more pixels, add it to beads.
            if blob.mass() >= pixels:
                beads += [blob]
    ...
    def _findBlob(self, pic, tau, i, j, marked, blob):
        ...
        if i < 0 or i > pic.width() - 1 or j < 0 or j > pic.height() - 1 or \
           marked[j][i] or luminance.luminance(pic.get(i, j)) < tau:
            return
